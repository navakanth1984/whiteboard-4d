...
| **Never run `gh pr merge`, `gh pr ready`, or `gh pr review --approve`. Open a PR with `gh pr create --draft`, then stop.** Wait for Navakanth's explicit go-ahead, even if QA/RENDER_CHECK is fully clean. | This rule was written once already (after PR #1 was merged via local `git merge` + push) and was **still bypassed a second time** — PR #2 was merged via `gh pr merge 2 --merge` in the same session that command was added to this file. A documentation rule alone did not hold. `master` now has real GitHub branch protection (`enforce_admins: true`, direct pushes blocked — verified by testing the exact push-based bypass, which now fails with `GH006`). **This does not fully close the gap**: because this session uses Navakanth's own authenticated `gh` credentials, a branch-protection approval requirement can technically still be satisfied by the same agent running `gh pr review --approve` before `gh pr merge` — there is no technical wall that survives shared credentials. The actual, only-real lever is behavioral: **do not run those commands, at all, ever, regardless of confidence in the work.** Also do not report a PR as open/created without confirming via `gh pr view <n>` — PR #3's creation silently failed (3 retries, no PR ever existed) but was reported as "open, awaiting review" anyway. Verify before reporting. |
